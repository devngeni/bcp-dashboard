...
const OperatorPages: NextPageWithLayout = () => {
  ...
  const handlePageChange = (newPage: number) => {
    router.push(`/operators?page=${newPage}`);
  ...
  useEffect(() => {
    if (currentPageParam) {
      ...
      if (!isNaN(parsedPage) && parsedPage >= 1 && parsedPage <= totalPages) {
        ...
        router.push("/operators?page=1 & page was not found"); // Redirect to page 1 by default
      }
    }
  ...
  const pageNavigateToQueryParam = ({
    ...
    if (product_id) {
      router.push(`/operators/${queryParam}?product_id=${product_id}`);
    } else {
      router.push(`/operators/${queryParam}`);
    }
  };
      ...
      <ProductsContainer>
        <TopLevel open={open}>
          ...
          >
            ...
            <YelloWButton
              onClick={() =>
                pageNavigateToQueryParam({ queryParam: "add-operator" })
              }
            >
              Add Operator
            </YelloWButton>
...
OperatorPages.getLayout = function getLayout(page: ReactElement) {
